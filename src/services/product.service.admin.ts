// services/product.service.admin.ts
...
export const AdminProductService = {
  ...
  async approveProduct(id: string, adminId: string) {
    ...
    if (product.approvalStatus === "ACTIVE") {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entity: "Product",
        entityId: id,
        oldData: { approvalStatus: product.approvalStatus },
        newData: { approvalStatus: "ACTIVE" },
      },
    });
  ...
  // Reject product
  // ======================
  async rejectProduct(id: string, adminId: string, reason?: string) {
    const product = await prisma.product.findUnique({
      where: { id },
      select: {
        name: true,
  ...
  // ======================
  // Filter products (admin view - all statuses)
  // ======================
  ...
  // ======================
  async bulkApprove(productIds: string[], adminId: string) {
  ...
  // ======================
  async bulkReject(productIds: string[], adminId: string, reason?: string) {
    const updated = await prisma.product.updateMany({
      where: {
        id: { in: productIds },
        approvalStatus: "PENDING",
