...
export const PublicProductService = {
 ...
 async filterProducts(
  ...
  // ... (all your existing where clause code remains the same)
  ...
  if (filters.attributes && Object.keys(filters.attributes).length > 0) {
    const attributeConditions = Object.entries(filters.attributes)
      .filter(([_, valueIds]) => Array.isArray(valueIds) && valueIds.length > 0)
          ...
          attributes: {
            some: {
              ...
              OR: [
                ...
                {
                  valueNumber: {
                    in: validValues.map((v) => parseFloat(v)).filter((v) => !isNaN(v)),
                  },
                },
                {
                  attributeValue: {
  ...
  try {
    ...
    if (filters.brands && filters.brands.length > 0) {
      filteredProducts = filteredProducts.filter((product: any) => {
        ...
        return brandAttributes.some((brand: string) => filters.brands!.includes(brand));
      });
    }
    ...
    if (filters.onSale) {
      filteredProducts = filteredProducts.filter((product: any) =>
        product.variants.some((v: any) => v.specialPrice && v.specialPrice < v.price)
      );
    }
